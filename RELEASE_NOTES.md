# @plus/pi-tool-display 0.1.0

The first standalone release of `@plus/pi-tool-display` introduces a rendering-only architecture for compact Pi tool rows and trustworthy tool-provided diffs.
...
This release moves presentation to Pi's final tool-row rendering seam. It never re-registers tools, wraps `execute`, replaces schemas, changes active tools, or mutates model/session data. Tool producers retain ownership and execution control; `@plus/pi-tool-display` only renders their existing calls and results.
